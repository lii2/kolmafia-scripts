...
string check_version(string soft, string proj, int thread) { buffer msg;
   ...
   vprint_html("Checking for updates (running <a href='https://kolmafia.us/showthread.php?t="+thread+"' target='_blank'>"+soft+"</a> rev. "+svn_info(proj).revision+")...",1);
   ...
   msg.append("<br><a href='https://kolmafia.us/showthread.php?t="+thread+"' target='_blank'><u>Click here "+
      "for discussion of what's new.</u></a> (<a href='https://kolmafia.us/showthread.php?goto=newpost&t="+thread+"' target='_blank'><u>last post</u></a>)");
...
string check_version(string soft, string prop, string thisver, int thread) { int w = 8; string page; matcher find_ver;
   ...
   string msg = "<big><font color=red><b>New Version of "+soft+" Available: "+zv[prop].ver+"</b></font></big>"+
      "<br><a href='https://kolmafia.us/showthread.php?t="+thread+"' target='_blank'><u>Upgrade from "+thisver+" to "+zv[prop].ver+" here!</u></a><br>";
...
boolean load_current_map(string fname, aggregate dest) {
   ...
   if (rem == "" || rem.length() > 150) return vprint("There was a problem accessing the Map Manager.",-1);  // also check length to make sure we didn't get something else
